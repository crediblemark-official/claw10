...
use clawhive_domain::{Approval, ApprovalState, ApprovalTargetType};
...
pub async fn list_approvals(
    ...
    let tool_approvals = state
        .kv_store
        .scan_prefix::<clawhive_domain::approval::ToolApprovalRequest>(TOOL_APPROVAL_PREFIX)
        .await?;
...
pub async fn approve_request(
    ...
    let mut tool_approval = state
        .kv_store
        .get::<clawhive_domain::approval::ToolApprovalRequest>(&tool_key)
        .await?
        .ok_or_else(|| ApiError::NotFound(format!("approval {id}")))?;

    if !matches!(
        tool_approval.state,
        clawhive_domain::approval::ToolApprovalState::Pending
    ) {
        return Err(ApiError::Validation(format!(
            "approval {id} is not pending"
        )));
    ...
    tool_approval.state = clawhive_domain::approval::ToolApprovalState::Approved;
...
pub async fn deny_request(
    ...
    tool_approval.state = clawhive_domain::approval::ToolApprovalState::Denied;
    state.kv_store.set(&tool_key, &tool_approval).await?;
