...
fn convert_model(provider_id: &str, entry: &ModelEntry) -> ModelProfile {
    let context = entry
        .limit
        .as_ref()
...
fn resolve_template_url(url: &str) -> String {
...
mod tests {
    ...
    fn test_resolve_template_url_missing_var() {
        ...
        // Empty replacement leaves double slashes
        assert!(resolve_template_url(url).contains("NONEXISTENT_VAR_12345") || resolve_template_url(url).contains("//"));
    }
